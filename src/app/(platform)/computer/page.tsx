...
export default function ComputerPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Fetch ONLY 'computer' quizzes
      const { data: quizData } = await supabase
        .from('quizzes')
        .select('*')
        .eq('quiz_type', 'computer')
  ...
  if (loading) return <div className="p-8 text-slate-500 font-bold animate-pulse">Loading Computer Quizzes...</div>

  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      <div className="mb-8 border-b border-slate-200 pb-6">
        <h1 className="text-3xl font-black text-slate-900 flex items-center">
          <span className="mr-3 text-4xl">💻</span> Computer Awareness
        </h1>
        <p className="text-slate-500 mt-2 text-lg">Master IT basics, networking, and software for banking exams.</p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {quizzes.map((quiz) => {
          ...
          return (
            <div key={quiz.id} className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden hover:border-teal-400 hover:shadow-md transition">
              <div className="p-6 flex-1">
                <div className="flex justify-between items-start mb-4">
                  <span className="px-3 py-1 bg-teal-50 text-teal-700 text-xs font-bold uppercase rounded-full">Computer</span>
              ...
              <div className="p-4 bg-slate-50 border-t border-slate-100">
                {isCompleted ? (
                  ...
                  <Link 
                    href={`/quiz/${quiz.id}`} 
                    className="block w-full text-center bg-teal-600 text-white py-2 rounded-lg font-bold hover:bg-teal-700 transition-colors shadow-sm"
                  >
                    Start Quiz
                  </Link>
                )}
        ...
        {quizzes.length === 0 && (
          <div className="col-span-full py-12 text-center text-slate-500 bg-white rounded-xl border border-dashed border-slate-300">
            No Computer Quizzes found. Add some from the Admin Panel!
          </div>
        )}
      </div>
    </div>
